...
const SidebarMenu: React.FC<SidebarMenuProps> = ({
  ...
  const minMenuHeight = 100;  // Altura contraída
  const maxMenuHeight = window.innerHeight * 0.8;  // Altura expandida
  ...
  return (
    <div
      className={`fixed bottom-0 left-0 w-full md:w-2/5 bg-white shadow-lg transition-all duration-300 ${isExpanded ? "shadow-lg" : ""
        }`}
      style={isMobileDevice ? { height: `${menuHeight}px`, maxHeight: "90vh" } : {}}
    >
      {/* Barra de agarre */}
      {isMobileDevice && (
        <div
          className="w-full flex justify-center items-center cursor-pointer py-2 bg-gray-200"
          onClick={toggleMenu}
